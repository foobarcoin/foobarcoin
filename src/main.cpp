...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // FBc: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "FBc Signed Message:\n";
...
{
    // FBc: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // FBc: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // FBc: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // 1 Oz of FBc: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // FBc: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // FBc: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // FBc: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Who died today, not me FBc !";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // FBc: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("FBcMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("FBcMiner : ProcessBlock, block not accepted");
    }
